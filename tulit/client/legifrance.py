...
def main():
    ...
    Examples:
        # Download a code (e.g., Code Civil)
        python -m tulit.client.legifrance --action download_code --text_id LEGITEXT000006070721 --dir ./data/france/legifrance --logdir ./logs
        
        # Download a legislative dossier
        python -m tulit.client.legifrance --action download_dossier --text_id LEGITEXT123456 --dir ./data/france/legifrance --logdir ./logs
        
        # Search for documents
        python -m tulit.client.legifrance --action search --query "droit du travail" --dir ./data/france/legifrance --logdir ./logs
...
Examples:
  # Download Code Civil
  %(prog)s --action download_code --text_id LEGITEXT000006070721
  
  # Search for documents
  %(prog)s --action search --query "droit du travail"
  
  # List codes with pagination
  %(prog)s --action list_codes --page_number 1 --page_size 10
        """
    )
    ...
    # Optional arguments
    parser.add_argument('--text_id', type=str, help='Text identifier (for code, dossier, etc.)')
    parser.add_argument('--article_id', type=str, help='Article identifier')
    parser.add_argument('--eli', type=str, help='European Legislation Identifier')
    parser.add_argument('--date', type=str, help='Date for versioned content (format: YYYY-MM-DD)')
    parser.add_argument('--query', type=str, help='Search query')
    parser.add_argument('--page_number', type=int, default=1, help='Page number for list operations (default: 1)')
    parser.add_argument('--page_size', type=int, default=10, help='Page size for list operations (default: 10)')
    parser.add_argument('--dir', type=str, default='./data/france/legifrance', help='Download directory')
    ...
    try:
        ...
        if args.action == 'download_code':
            ...
            if not args.text_id:
                parser.error("--text_id is required for download_dossier action")
            result = client.download_dossier_legislatif(args.text_id)
            print(f"Dossier downloaded to: {result}")
            
        elif args.action == 'consult_code':
            if not args.text_id:
                parser.error("--text_id is required for consult_code action")
            result = client.consult_code(args.text_id, args.date)
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
        elif args.action == 'consult_article':
            if not args.article_id:
                parser.error("--article_id is required for consult_article action")
            result = client.consult_article(args.article_id, args.date)
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
        elif args.action == 'consult_dossier':
            if not args.text_id:
                parser.error("--text_id is required for consult_dossier action")
            result = client.consult_dossier_legislatif(args.text_id)
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
        elif args.action == 'list_codes':
            result = client.list_codes(args.page_number, args.page_size, args.date)
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
        elif args.action == 'list_loda':
            result = client.list_loda(args.page_number, args.page_size, args.date)
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
        elif args.action == 'list_dossiers':
            result = client.list_dossiers_legislatifs(args.page_number, args.page_size)
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
        elif args.action == 'search':
            if not args.query:
                parser.error("--query is required for search action")
            result = client.search(args.query, args.page_number, args.page_size)
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
        elif args.action == 'suggest':
            if not args.query:
                parser.error("--query is required for suggest action")
            result = client.suggest(args.query)
            print(json.dumps(result, indent=2, ensure_ascii=False))
        
        client.logger.info(f"Action '{args.action}' completed successfully")
        
    except Exception as e:
        client.logger.error(f"Error executing action '{args.action}': {e}")
