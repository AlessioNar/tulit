"""
End-to-end tests for German legislation parser.
Tests the complete parsing pipeline from downloaded German legislation files to parsed JSON output.
Uses files downloaded by German legislation client e2e tests as fixtures.
"""
...
@pytest.mark.e2e
@pytest.mark.parser
class TestGermanLegislationParser:
    """Test suite for German legislation parser functionality."""

    @pytest.mark.slow
    def test_german_legislation_parsing(self, db_paths):
        """Test parsing German legislation XML documents."""
        sources_dir = db_paths['sources'] / 'member_states' / 'germany'
        results_dir = db_paths['results'] / 'member_states' / 'germany'
        logs_dir = db_paths['logs']

        # Find downloaded XML files
        xml_files = list(sources_dir.glob('*.xml'))
        if not xml_files:
            pytest.skip("No German legislation XML files found - run German legislation client e2e tests first")
