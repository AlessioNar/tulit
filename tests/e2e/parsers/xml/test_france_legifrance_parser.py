"""
End-to-end tests for France Legifrance parser.
Tests the complete parsing pipeline from downloaded Legifrance files to parsed JSON output.
Uses files downloaded by France Legifrance client e2e tests as fixtures.
"""
...
@pytest.mark.e2e
@pytest.mark.parser
class TestFranceLegifranceParser:
    """Test suite for France Legifrance parser functionality."""
    ...
    def test_france_legifrance_parsing(self, db_paths):
        ...
        if not xml_files:
            pytest.skip("No France Legifrance XML files found - run France Legifrance client e2e tests first")
