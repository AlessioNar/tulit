"""
End-to-end tests for Italy Normattiva AKN parser.
Tests the complete parsing pipeline from downloaded Normattiva files to parsed JSON output.
Uses files downloaded by Italy Normattiva client e2e tests as fixtures.
"""
...
@pytest.mark.e2e
@pytest.mark.parser
class TestItalyNormattivaParser:
    """Test suite for Italy Normattiva AKN parser functionality."""
    ...
    def test_italy_normattiva_parsing(self, db_paths):
        ...
        if not xml_files:
            pytest.skip("No Italy Normattiva XML files found - run Italy Normattiva client e2e tests first")
